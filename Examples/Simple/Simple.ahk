...
for row, data in Ex4_Table1 {
	...
	for col, cell in data
		html .= NeutronWindow.FormatHTML("<td>{}</td>", cell)
	html .= "</tr>"
}
...
#HotIf WinActive("ahk_id" neutron.gui.hWnd)
...
UpdateKeyExample(neutron, keyName, className) {
	...
	; for loop.
	for i, div in NeutronWindow.Each(keyDivs) {
...
Ex4_Submit1(neutron, event) {
	...
	html := NeutronWindow.FormatHTML("<tr><td>{}</td><td>{}</td>", formData.ex4_item1, formData.ex4_cost1)
